...
export default function Nav() {
  ...
  return (
    <>
      ...
      {/* Mobile bottom bar */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur border-t border-slate-800 flex z-50">
        {NAV_LINKS.map(({ href, labelKey, icon }) => {
          const active = pathname === href || pathname.startsWith(href + '/');
          return (
            <Link key={href} href={href}
        ...
        {/* More menu: language + sign out */}
        <button
          onClick={() => setLanguage(otherLang)}
          className="flex-1 flex flex-col items-center gap-0.5 py-2.5 text-xs font-medium text-slate-500 transition-colors"
        >
          <span className="text-base leading-none">{language === 'en' ? '🇩🇰' : '🇬🇧'}</span>
          <span>{language === 'en' ? 'DA' : 'EN'}</span>
        </button>
        <button
          onClick={signOut}
          className="flex-1 flex flex-col items-center gap-0.5 py-2.5 text-xs font-medium text-slate-500 hover:text-red-400 transition-colors"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
          </svg>
